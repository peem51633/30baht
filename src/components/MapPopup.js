...
const MapPopup = ({ feature, onClose, popupInfo }) => {
    ...
    // ฟังก์ชันสำหรับแชร์ตำแหน่งไปเปิดใน Google Map พร้อมปักหมุด
    const handleOpenGoogleMap = () => {
        let latitude = data?.lat || data?.latitude || data?.Latitude;
        let longitude = data?.long || data?.longitude || data?.Longitude || data?.Lng;
        
        if (!latitude || !longitude) {
            alert('ไม่พบข้อมูลพิกัด (lat/long) สำหรับตำแหน่งนี้');
            return;
        }
    ...
    return (
        ...
        >
            ...
            <button onClick={(e) => { handleButtonClick(e); handleOpenGoogleMap(); }} title="เปิดใน Google Map" style={{
                position: 'absolute',
                top: '8px',
